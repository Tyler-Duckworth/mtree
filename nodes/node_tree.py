...
"""
This file defines the Node Tree that will spawn 
"""
            ...
            if node.bl_idname != "MtreeTwig": # twig nodes are not saved (mayby they should ?)
                ...
                for prop in node.properties:
                    if type(getattr(node, prop)) != Object:
                        ...
                        node_data[prop] = True
            if node.bl_idname == "MtreeParameters":
                if node_data["create_leafs"] == False:
                    node_data["leaf_dupli_object"] = None
...
class MTREE_OT_AppendMaterials(Operator):
    """Import Bark materials"""
    bl_idname = "mtree.appendmaterials"
...
class MTREE_PT_Panel(bpy.types.Panel):
    bl_idname = "MTREE_PT_Panel"
    bl_label = "Mtree"
    ...
    def draw(self, context):
        ...
        op = box.operator("mtree.saveloadpreset", text='load preset') # will call MtreeSavePreset.execute
        ...
        op = box.operator("mtree.saveloadpreset", text='save preset') # will call MtreeSavePreset.execute
        op.node_group_name = tree.name #set node group name as curent node tree name
        op.load = False # set action to save

        box = layout.box()
        box.label(text="materials")
        box.operator("mtree.appendmaterials", text="append materials")
...
classes = [
    ...
    TreeSocket,
    MTREE_PT_Panel,
    MTREE_OT_SaveLoadPreset,
    MTREE_OT_AppendMaterials,
]
