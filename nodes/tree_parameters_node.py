...
class MtreeParameters(Node, BaseNode):
    ...
    def draw_buttons(self, context, layout):
        ...
        if active_tree: # if a tree object is locked
            ...
            op = split.operator("mtree.resetactivetree", text='x')
        ...
        if need_update and not self.auto_update: # if tree needs to be updated
            ...
            op = layout.operator("mtree.executenodetree", text=prop_text) # will call ExecuteMtreeNodeTreeOperator.execute
        ...
        op = layout.operator("mtree.randomizetree", text='randomize tree') # will call RandomizeTreeOperator.execute
        op.node_group_name = self.id_data.name # set name of node group to operator
...
class MTREE_OT_ExecuteNodeTree(Operator):
    """Execute or update tree"""
    bl_idname = "mtree.executenodetree"
...
class MTREE_OT_Randomize(Operator):
    """Randomize all tree seeds"""
    bl_idname = "mtree.randomizetree"
...
class MTREE_OT_ResetActiveTree(Operator):
    """stop tracking tree"""
    bl_idname = "mtree.resetactivetree"
