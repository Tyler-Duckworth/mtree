...
class MtreeTrunk(Node, BaseNode):
    ...
    radius_randomness: FloatProperty(min=0, default=0, update=BaseNode.property_changed)
    ...
    properties = ["seed", "length", "radius", "end_radius", "radius_randomness", "resolution", "shape", "randomness", "axis_attraction", "use_grease_pencil"]
    ...
    def draw_buttons(self, context, layout):  
        ...
        if self.use_grease_pencil:
            ...
            op = layout.operator("mtree.updategreasepencil", text="update strokes") # will call ExecuteMtreeNodeTreeOperator.execute
    ...
    def execute(self, tree):
        ...
        if self.use_grease_pencil:
            ...
            tree.add_trunk(self.length, self.radius, self.end_radius, self.radius_randomness, self.shape, self.resolution, self.randomness, self.axis_attraction, creator)
...
class MTREE_OT_UpdateGreasePencil(Operator):
    """Update grease pencil strokes"""
    bl_idname = "mtree.updategreasepencil"
